...
@app.get(BOARDS_ENDPOINT, response_class=ORJSONResponse)
def boards():
...
@app.get(THREADLIST_ENDPOINT, response_class=ORJSONResponse)
...
@app.get(CATALOG_ENDPOINT, response_class=ORJSONResponse)
...
@app.get(ARCHIVE_ENDPOINT, response_class=ORJSONResponse)
...
@app.get(INDEXES_ENDPOINT, response_class=ORJSONResponse)
def index(board_name: str, page_num: int):
    return get_fourchan_json(
        INDEXES_ENDPOINT.format(board_name=board_name, page_num=page_num)
    )
...
@app.get(THREAD_ENDPOINT, response_class=ORJSONResponse)
def thread(board_name: str, thread_id: int):
    ...
    return get_fourchan_json(
        THREAD_ENDPOINT.format(board_name=board_name, thread_id=thread_id)
    )
...
# HTML endpoints
#
@app.get("/{board_name}/catalog.html", response_class=HTMLResponse)
...
def thread_html(board_name: str, thread_id: int):
    ...
    return HTMLResponse(
        content=template.render(
            posts=thread_dict["posts"], board=board_name, title=title
        ),
        status_code=200,
    )
